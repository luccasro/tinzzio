...
export const Room = ({ route }: NativeStackHeaderProps) => {
  ...
  const [roomId, setRoomId] = useState<RoomId | undefined>(
    route.params as RoomId,
  );
  const [userLoggedId, setUserLoggedId] = useState<string | undefined>(
    undefined,
  );
    ...
    if (room !== undefined && userLoggedId) {
      if (room.participants && room.participants.length > 0) {
        const userIndex = room.participants.findIndex(
          (it) => it.id === userLoggedId,
        );

        if (userIndex !== -1 && userIndex !== undefined) {
          if (room.participants[userIndex]?.answers) {
            setAnsweredQuestion(
              room.participants[userIndex].answers as QuestionValues[],
            );
            setFinished(true);
            setRoomExists(true);
          }
        ...
        {room && roomExists ? (
          <>
            {!room.disabled ? (
              <>
                {!finished ? (
                            ...
                            {currentIndex === index && (
                              <>
                                <VStack>
                                  ...
                                  <Box p="2" bg="purple.600" borderRadius="sm">
                                    <Text
                                      fontSize={[16, 30]}
                                      bold
                                      color="white"
                                ...
                                >
                                  {question.a && (
                                    <Flex w="50%" mb={4} pr={[2, 4]}>
                                      <Box w="100%" justifyContent="center">
                                        <Pressable
                                          onPress={() =>
                                            checkQuestion('a', index)
                                          }
                                        >
                                          {({ isHovered, isPressed }) => {
                                            return (
                                ...
                                <Button
                                  ...
                                  isDisabled={
                                    answeredQuestion[index] === undefined
                                  }
                                >
                  ...
                  <>
                    ...
                    <Text>
                      {t('room.correctAnswers', {
                        correctCount: answeredQuestion.filter(
                          (it) => it.isCorrect,
                        ).length,
                    ...
                    <Text>
                      {t('room.percentage')}{' '}
                      {(answeredQuestion.filter((it) => it.isCorrect).length /
                    ...
                    {room.questions.map((question, index) => {
                      return (
                        <Box key={index} mt={5}>
                          ...
                          {answeredQuestion &&
                          answeredQuestion[index].isCorrect === true ? (
                            ...
                            answeredQuestion &&
                            answeredQuestion[index].isCorrect === false && (
              ...
              <Text>{t('room.disabledQuiz')}</Text>
            )}
          </>
        ) : roomExists === false ? (
          <>
            <Text fontSize={22}> {t('room.invalidQuiz')}</Text>
            <Button mt={4} onPress={goBack}>
              {t('room.backDashboard')}
            </Button>
          </>
        ) : (
